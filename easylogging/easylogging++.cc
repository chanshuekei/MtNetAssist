...
void TypedConfigurations::rebuildFilename(Configurations* configurations) {
    for (Configurations::const_iterator it = configurations->begin(); it != configurations->end(); ++it) {
      Configuration* conf = *it;
      if (conf->configurationType() == ConfigurationType::Filename) {
          base::type::fstream_t* fs = unsafeGetConfigByRef(conf->level(), &m_fileStreamMap, "fileStream").get();
          if (fs == nullptr) {
              continue;
          }

          std::string& fname = unsafeGetConfigByRef(conf->level(), &m_filenameMap, "filename");
          std::string resolvedFilename = resolveFilename(conf->value());
          if (fname == resolvedFilename) {
              continue;
          }
          fname = resolvedFilename;
          fs->close();
          insertFile(conf->level(), conf->value());
          fs->open(resolvedFilename, std::fstream::out | std::fstream::ate | std::fstream::app);
      }
    }
}
...
bool TypedConfigurations::unsafeValidateFileRolling(Level level, const PreRollOutCallback& preRollOutCallback) {
  ...
  rebuildFilename(m_configurations);
